...
// Configura o Virtual File System (vfs) para o pdfMake com múltiplas tentativas
try {
...
// Export the configured pdfMake instance
export { pdfMake, logo };
...
const headerFunction = (currentPage, pageCount, etapa) => {
    ...
    columns: [
      { image: logo, width: 130, margin: [40, 15, 0, 0] },
      {
        stack: [
          ...
          { text: `CIDADE: ${etapa?.cidade ? etapa.cidade.toUpperCase() : 'TUPÃ'}`, style: 'eventInfo', alignment: 'right' },
...
const buildBalizamentoDocDefinition = (dados, etapa, compactLayout) => {
  ...
  const formattedData = Object.keys(dados).map(prova => {
    ...
    return {
      nome: nomeProvaFormatado, // Use o nome formatado
      isRevezamento,
  ...
  return {
    ...
    content: [
      ...
      ...formattedData.map((prova, provaIndex) => [
        { text: `${provaIndex + 1}ª Prova: ${prova.nome}`, style: 'subheader', margin: provaIndex === 0 ? [0, 0, 0, 0] : [0, 20, 0, 0] },
          ...
          },
          {
            table: {
              ...
              widths: prova.isRevezamento
                ? [16, 215, 80] 
                : [16, 180, 45, 215, 40], // Larguras: "Raia" = 16, "Nome" = 160, "Categoria" = 50, "Equipe" = 205
              body: [
                prova.isRevezamento
                  ? [
                ...
                ...(
                  prova.isRevezamento
                    ? bateria.map(nadador => [
                        { text: nadador.raia, style: 'tableData' },
                        { text: nadador.equipe, style: 'tableData' },
                        { text: nadador.melhor_tempo, style: 'tableData' }
                      ])
                    : bateria.map(nadador => [
                        { text: nadador.raia, style: 'tableData' },
                        { text: nadador.nome_nadador, style: 'tableData' },
                        { text: nadador.categoria, style: 'tableData' },
                        { text: nadador.equipe, style: 'tableData' },
                        { text: nadador.melhor_tempo, style: 'tableData' }
                      ])
                )
    ...
    styles: {
      ...
      tableData: { // estilo para os dados da tabela
        fontSize: 8,
...
export const gerarFilipetas = (dadosBalizamento, evento) => {
  // Ensure `dadosBalizamento` and `evento` are valid
  if (!dadosBalizamento || typeof dadosBalizamento !== 'object' || !Object.keys(dadosBalizamento).length) {
    console.error('Dados de balizamento inválidos para gerar filipetas.');
    return;
  }

  if (!evento || typeof evento !== 'object') {
    console.error('Evento inválido para gerar filipetas.');
    return;
  }
  ...
  Object.keys(dadosBalizamento).forEach((tipoProva) => {
    const tipoProvaFormatado = tipoProva.replace(/\bF\b/, 'FEMININO').replace(/\bM\b/, 'MASCULINO'); // Substituição adicionada
    dadosBalizamento[tipoProva].forEach((bateria, indiceBateria) => {
      bateria.flat().forEach((nadador) => {
        nadadoresData.push({
          tipoProva: tipoProvaFormatado, // Use o nome formatado
  ...
  const docDefinition = {
    ...
    content: pages.length ? pages : [{ text: 'Nenhum dado disponível para gerar filipetas.', style: 'header' }],
    styles: {
      tableHeader: {
        bold: true,
        fontSize: 10,
  ...
  // Ensure `docDefinition` is valid before generating the PDF
  if (!docDefinition || !docDefinition.content || !docDefinition.content.length) {
    console.error('Definição do documento inválida para gerar filipetas.');
    return;
  }
