...
const calcularRankingNadadores = async (conn, torneiosId) => {
    try {
        const [resultados] = await conn.execute(
            `SELECT c.nadadores_id, n.categorias_id, SUM(c.pontuacao_individual) AS total_pontos, ep.eventos_id, c.eventos_provas_id
             FROM classificacoes c
             JOIN eventos_provas ep ON c.eventos_provas_id = ep.id
             JOIN eventos e ON ep.eventos_id = e.id
             JOIN nadadores n ON c.nadadores_id = n.id
             WHERE e.torneios_id = ?
             GROUP BY c.nadadores_id, n.categorias_id, c.eventos_provas_id`,
            [torneiosId]
        );

        // Atualiza rankingNadadores
        for (const row of resultados) {
            await conn.execute(
                `INSERT INTO rankingNadadores (torneios_id, eventos_id, eventos_provas_id, nadadores_id, categorias_id, pontos)
                 VALUES (?, ?, ?, ?, ?, ?)
                 ON DUPLICATE KEY UPDATE pontos = pontos + VALUES(pontos)`,
                [torneiosId, row.eventos_id, row.eventos_provas_id, row.nadadores_id, row.categorias_id, row.total_pontos]
            );
        }
    } catch (error) {
        console.error("Erro ao calcular ranking de nadadores:", error);
    }
};
...
router.get('/ranking-nadadores/:torneiosId', async (req, res) => {
    try {
        ...
        const [masculinoRanking] = await db.execute(
            `SELECT n.nome AS nadador, c.nome AS categoria, r.pontos 
             ...
             ORDER BY c.nome ASC, r.pontos DESC`,
            [torneiosId]
        );
